...
const CartPage = () => {
  ...
  const handleSubmitOrder = () => {
    ...
    setTimeout(() => {
      ...
      // Add WhatsApp confirmation button with detailed order information
      let whatsappMessage = `طلب جديد #${orderNumber}\n\n`;
      whatsappMessage += `الاسم: ${customerInfo.name}\n`;
      whatsappMessage += `الهاتف: ${customerInfo.phone}\n`;
      whatsappMessage += `العنوان: ${customerInfo.address}\n\n`;

      // Add order items
      whatsappMessage += `المنتجات:\n`;
      cartItems.forEach((item) => {
        whatsappMessage += `- ${item.name} x${item.quantity} (${item.discountPrice * item.quantity} د.م)\n`;
        if (item.extras && item.extras.length > 0) {
          item.extras.forEach((extra) => {
            whatsappMessage += `  + ${extra.name} (${extra.price * item.quantity} د.م)\n`;
          });
        }
      });

      whatsappMessage += `\nالمجموع الفرعي: ${calculateSubtotal()} د.م\n`;
      whatsappMessage += `الإضافات: ${calculateExtrasTotal()} د.م\n`;
      whatsappMessage += `الخصم: ${calculateDiscount()} د.م\n`;
      whatsappMessage += `المجموع: ${calculateTotal()} د.م\n\n`;

      // Add delivery time
      whatsappMessage += `وقت التوصيل: ${deliveryTime === "asap" ? "أسرع وقت ممكن" : scheduledTime}\n`;
      whatsappMessage += `طريقة الدفع: ${paymentMethod === "cash" ? "الدفع عند الاستلام" : "رصيد الكوينز"}\n\n`;

      // Add loyalty program details if selected
      if (selectedLoyaltyCard) {
        whatsappMessage += `برنامج الولاء:\n`;
        if (selectedLoyaltyCard.type === "reward") {
          if (loyaltyAction === "redeem") {
            whatsappMessage += `تم استخدام مكافأة: ${selectedLoyaltyCard.stamps && selectedLoyaltyCard.stamps[0].reward}\n`;
          } else if (loyaltyAction === "request") {
            whatsappMessage += `طلب طابع جديد\n`;
          }
        } else if (selectedLoyaltyCard.type === "gift") {
          whatsappMessage += `تم استخدام بطاقة هدية: ${selectedLoyaltyCard.title}\n`;
        }
        whatsappMessage += `رقم البطاقة: ${selectedLoyaltyCard.code}\n`;
      }

      summary += `<div class="border-t pt-4 mt-4">
        <a href="https://wa.me/212675812554?text=${encodeURIComponent(whatsappMessage)}" target="_blank" class="flex items-center justify-center gap-2 w-full p-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition-colors">
